...
window.addEventListener("DOMContentLoaded", function () {
  ...
  var htmlContents = document.querySelectorAll("[data-content]");

  var generateBanner = function generateBanner(object) {
    var pricePerMonth = "$9.99";
    var pricePerYear = "$19.99";
    var pricePerMonthPerYear = "$" + Math.floor(pricePerYear.slice(1) / 12 * 100) / 100;
    htmlContents.forEach(function (content) {
      content.innerHTML = object[content.dataset.content];

      if (content.dataset.content.includes("{{price}}")) {
        var priceText = content.textContent.slice(9);

        if (content.dataset.content.includes("per month")) {
          content.innerHTML = "<strong>".concat(pricePerMonth, "</strong><br>").concat(priceText);
          return;
        }

        if (content.dataset.content.includes("per year")) {
          content.innerHTML = "<strong>".concat(pricePerYear, "</strong><br>").concat(priceText);
          return;
        }

        if (content.dataset.cost === 'pricePerMonth') {
          content.innerHTML = "".concat(pricePerMonth).concat(priceText);
          return;
        }

        if (content.dataset.cost === 'pricePerMonthPerYear') {
          content.innerHTML = "".concat(pricePerMonthPerYear).concat(priceText);
          return;
        }
      }
    });
